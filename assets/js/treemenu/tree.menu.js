...
TreeMenu.prototype.setup_symbols = function() {
	...
	var index = 0;
	var ul, li, symbol, islast = false, a;
	var ul_elements, li_elements = this.top_ul.getElementsByTagName("LI"), a_elements = this.top_ul.getElementsByTagName("A");
	for(var i=0; i < li_elements.length; i++) {
		...
		if(ul_elements.length > 0) {
			...
			if (this.SymbolTag && ! this.OmitSymbols) {
				...
				a.insertBefore(symbol, a.firstChild);
			}
			...
			if (this.ClassItem) a.className += ' ' + this.ClassItem;
		}

		if (islast) a.className += ' ' + this.ClassLast;
	}
}
